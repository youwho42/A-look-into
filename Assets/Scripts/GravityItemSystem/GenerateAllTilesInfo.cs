...
public class GenerateAllTilesInfo : MonoBehaviour
{
    ...
    void GetAllSurroundingPositions()
    {
                        ...
                        if (!thing.isValid)
                        {
                            ...
                            if (z < 0)
                            {
                                if (valid || thing.tileName == "Empty")
                                    thing = new TileDirectionInfo(dir, valid, z, tileName);
                            }
                            else
                                continue;
                        }
        ...
        for (int i = 0; i < allDirectionsValues.Count; i++)
        {
            if (allDirectionsValues[1].tileName.Contains("Slope"))
            {
                Vector3Int tileDirection = allDirectionsValues[1].tileName.Contains("X") ? Vector3Int.right : Vector3Int.up;
                tileDirection = allDirectionsValues[1].tileName.Contains("1") ? tileDirection : -tileDirection;
                allDirectionsValues[1].isValid = true;
            }
        }
