...
namespace Klaxon.SAP
{
    public class SAP_Action_Sit : SAP_Action
    {
        ...
        public override void StartPerformAction(SAP_Scheduler agent)
        {
        ...
        public override void PerformAction(SAP_Scheduler agent)
        {
        ...
        public override void EndPerformAction(SAP_Scheduler agent)
        {
            agent.lastValidNode = currentNode;
        ...
        private void HandleOffScreen(SAP_Scheduler agent)
        {
            agent.walker.currentDir = Vector2.zero;
            int frameSkip = 60;
            if (currentPathIndex < path.Count - 1)
        ...
        void Deviate(SAP_Scheduler agent)
        {
            isDeviating = true;
        ...
        void ReachFinalDestination(SAP_Scheduler agent)
        {
        ...
        IEnumerator PlaceNPC(SAP_Scheduler agent, Vector3 position)
        {
            float timer = 0;
            float maxTime = 0.45f;
            while (timer < maxTime)
