...
public class DragonflyAI : MonoBehaviour, IAnimal
{
        ...
        switch (currentState)
        {
            case FlyingState.isFlying:
                ...
                if (!justTookOff /*&& CheckVisibility()*/)
                    CheckForLandingArea();
                if (Vector2.Distance(transform.position, flight.currentDestination) <= 0.015f)
                {
                    timeToStayAtDestination = SetTimeToStayAtDestination(0.1f, 2.0f);
                    currentState = FlyingState.isHovering;
                }
                    break;

            case FlyingState.isHovering:
            ...
            case FlyingState.isLanding:
                ...
                if (Vector2.Distance(transform.position, flight.currentDestination) <= 0.01f && Vector2.Distance(gravityItem.itemObject.localPosition, flight.currentDestinationZ) <= 0.01f)
                {
                    ...
                    timeToStayAtDestination = SetTimeToStayAtDestination(0.5f, 10.0f);
    ...
    float SetTimeToStayAtDestination(float min, float max)
    {
