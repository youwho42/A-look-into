...
public class MazeStringGame : MonoBehaviour
{
    ...
    private void Update()
    {
        if(inMaze && line.positionCount > 1 && !mazeComplete) 
            line.SetPosition(line.positionCount - 1, PlayerInformation.instance.playerTransform.position + new Vector3(0, GlobalSettings.SpriteDisplacementY * 0.6f, endPostItems[currentIndex].transform.position.z + 3));
    }
